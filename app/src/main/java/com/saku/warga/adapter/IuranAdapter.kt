...
class IuranAdapter(private val context: Context) : RecyclerView.Adapter<IuranAdapter.NamaKelompokViewHolder>() {
    private val dataArray= mutableListOf<RiwayatIuranData>()
    val library=Library()
    fun addData(data:MutableList<RiwayatIuranData>){
        dataArray.addAll(data)
        notifyDataSetChanged()
    }

    fun initData(data:MutableList<RiwayatIuranData>){
        dataArray.clear()
        dataArray.addAll(data)
        dataArray.sortByDescending { it.tgl }
        notifyDataSetChanged()
    }
...
//        holder.title.text = dataArray[position].keterangan
        holder.title.text = "Pembayaran Iuran"
